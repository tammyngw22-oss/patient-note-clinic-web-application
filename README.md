# Nightingale Clinic - Patient Note Web Application
...
*   **LLM API**: Real-time AI analysis requires a valid API Key (e.g., Gemini); otherwise, the system falls back to pre-generated simulation data.
*   **Patient Glance View**: Currently, the Patient role does not have a Glance View (by design).
*   **Timeline Format**: currently presented as a chronological feed; future versions may group items into "Episode Cards".
*   **Diff / Plan**: Revision history is tracked and viewable, but complex "diff" visualization (showing exact text changes) is a future enhancement.
